...
namespace UserRegistrationSolution
{
    internal class UserRegistrationWithoutLambdaExpression
    {
        ...
        public bool RegexCheck(string firstName, string lastName, string email)
        {
            return (Regex.IsMatch(firstName, FIRST_NAME_PATTERN) && Regex.IsMatch(lastName, LAST_NAME_PATTERN) && Regex.IsMatch(email, EMAIL_PATTERN)) ? true : false;
        }
    }
}